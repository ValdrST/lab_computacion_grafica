...
int main() 
{
	...
	Helice_lateral.LoadModel("Models/helice_lateral.obj");
	Model Pelota = Model();
...
//contador de luces puntuales
	...
	GLfloat rot_y = 0.0f;
	GLfloat rebote = 2.0f;
	GLfloat pos_y_func = 0.0f;
	GLfloat rebote_y_func = 0.0f;
	GLfloat rot_pelota = 0.0f;
	GLfloat alt_var = 1.0f;
	GLfloat max_rebote = 2.0f;
	GLfloat alt_pelota = 0.0f;
	GLfloat rot_helice = 0.0f;
	movOffset = 1.0f;
	bool bandera_sentido = false;
	GLint bandera_rebote = 0;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		deltaTime = 5.0f*(now - lastTime); 
		lastTime = now;
		// Animacion compleja de helicoptero
		if(pos_x <= 20.0f && !bandera_sentido ){
			pos_x += movOffset * deltaTime;
			if(rot_y > 0.0f){
				rot_y -= 45.0f * deltaTime;
			}
		}else{
			bandera_sentido = true;
			//rot_y = 180.0f;
		} if(pos_x >= 0.0f && bandera_sentido){
			pos_x -= movOffset * deltaTime;
			if(rot_y < 180.0f){
				rot_y += 45.0f * deltaTime;
			}
			...
			//rot_y = 0.0f;
			bandera_sentido = false;
		}
		if(pos_y_func <= 360.0f){
			rot_pelota += 45.0f * deltaTime;
			pos_y_func += 45.0f * deltaTime;
		}else{
			rot_pelota = 0.0f;
			pos_y_func = 0.0f;
		}
		if(rot_helice <= 360.0f){
			rot_helice += 90 * deltaTime;
		}else{
			rot_helice = 0.0f;
		}
		pos_y = alt_var*(glm::sin(glm::radians(pos_y_func)));

		// Animacion rebote de pelota
		if(rebote_y_func <= 180.0f){
			rebote_y_func += 30.0f * deltaTime;
		}else{
			bandera_rebote++;
			rebote_y_func = 0.0f;
		}
		if(alt_pelota > 0){
			alt_pelota = max_rebote - (bandera_rebote * 0.5f);
		}else{
			bandera_rebote = 0;
			alt_pelota = max_rebote - (bandera_rebote * 0.5f);
		}
		rebote = alt_pelota*(glm::sin(glm::radians(rebote_y_func)));
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f,-1.9f+rebote,0.0f));
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
		model = glm::rotate(model, glm::radians(rot_pelota), glm::vec3(0.0f, 1.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		Pelota.RenderModel();

		model = glm::mat4(1.0);
		posblackhawk = glm::vec3(pos_x, 2.0f+pos_y, 0.0f);
		model = glm::translate(model, posblackhawk);
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
		model = glm::rotate(model, -90* toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, -90 * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::rotate(model, glm::radians(rot_y), glm::vec3(0.0f, 0.0f, 1.0f));
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		...
		Helice.RenderModel();
		model = modelaux;
		model = glm::translate(model,glm::vec3(0.0f,-10.50f,2.0f));
		model = glm::rotate(model, glm::radians(rot_helice), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
