	...
	for (size_t i = 0; i < scene ->mNumMaterials; i++){
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE	)){
			...
			if (material->GetTexture(aiTextureType_DIFFUSE,0,&path)==AI_SUCCESS){
				...
				bool statusTexture = TextureList[i]->LoadTexture();
				bool statusTextureA = false;
				if(!statusTexture){
					statusTextureA = TextureList[i]->LoadTextureA();
					printf("Textura alfa\n");
				}
				if (!(statusTextureA || statusTexture)){
